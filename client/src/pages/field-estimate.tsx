...
export default function FieldEstimate() {
  ...
  const syncMaterialFromRecoveryArea = () => {
    ...
    materialRows.forEach(row => {
      if ((row.isAutoGenerated || row.isLinkedFromRecovery) && row.autoKey) {
        existingAutoRowsMap.set(row.autoKey, row);
      }
    });
    ...
    // 결과 병합: 업데이트된 자동 행 + 독립 행
    const resultAutoRows = Array.from(resultRowsMap.values());
    const allRows = [...resultAutoRows, ...independentRows];
    ...
    if (allRows.length > 0) {
      ...
      toast({
        title: "자재비 동기화 완료",
        description: `복구면적 산출표에서 ${resultAutoRows.length}개 항목이 자동 생성되었습니다.`,
      });
    }
  };
