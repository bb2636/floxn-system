...
export default function FieldEstimate() {
  ...
  // 공사명 기준으로 reconcile하여 저장/로드 후에도 동작
  // laborCostRows 포함 (수동 삭제 시 재생성 위해) + 안정적인 키 비교로 무한 루프 방지
  ...
  useEffect(() => {
    ...
    // 철거공사 필요한 공사명 목록 (일위대가DB 철거공사에 있는 공사명)
    const DEMOLITION_WORK_NAMES = ['합판', '반자틀', '석고보드', '도배', '마루', '장판'];
    
    // 1. 복구면적 산출표에서 철거공사가 필요한 공사명 추출 (중복 제거)
    const requiredDemolitionWorkNames = new Set<string>();
    rows.forEach(row => {
      if (row.workType && row.workName && row.workType !== '철거공사') {
        const normalizedWorkName = normalizeForMatch(row.workName);
        const needsDemolition = DEMOLITION_WORK_NAMES.some(
          name => normalizeForMatch(name) === normalizedWorkName
        );
        if (needsDemolition) {
          requiredDemolitionWorkNames.add(row.workName);
        }
      }
    });
    
    // 2. 현재 노무비에서 철거공사 행의 공사명 추출 (자동/수동 모두 포함)
    const existingDemolitionWorkNames = new Set<string>();
    const autoGeneratedDemolitionWorkNames = new Set<string>();
    laborCostRows.forEach(row => {
      if (row.category === '철거공사') {
        existingDemolitionWorkNames.add(row.workName);
        if (row.isLinkedFromRecovery) {
          autoGeneratedDemolitionWorkNames.add(row.workName);
        }
      }
    });
    
    // 3. 누락된 철거공사 공사명 찾기 (복구면적에 있지만 노무비에 아예 없는 것)
    const missingDemolitionWorkNames: string[] = [];
    requiredDemolitionWorkNames.forEach(workName => {
      if (!existingDemolitionWorkNames.has(workName)) {
        missingDemolitionWorkNames.push(workName);
      }
    });
    
    // 4. 더 이상 필요 없는 자동생성 철거공사 공사명 찾기
    const orphanedDemolitionWorkNames: string[] = [];
    autoGeneratedDemolitionWorkNames.forEach(workName => {
      if (!requiredDemolitionWorkNames.has(workName)) {
        orphanedDemolitionWorkNames.push(workName);
      }
    });
    
    // 무한 루프 방지: 필요 + 현재 상태를 모두 포함한 키로 비교
    // required:missing:orphaned 형식으로 안정적인 비교
    const stateKey = [
      Array.from(requiredDemolitionWorkNames).sort().join(','),
      missingDemolitionWorkNames.sort().join(','),
      orphanedDemolitionWorkNames.sort().join(',')
    ].join('|');
    
    // 변경이 없으면 조기 종료 (무한 루프 방지)
    if (demolitionReconcileRef.current === stateKey) {
      return;
    }
    
    // 실제 변경이 없으면 조기 종료 (setLaborCostRows 호출 방지)
    if (missingDemolitionWorkNames.length === 0 && orphanedDemolitionWorkNames.length === 0) {
      demolitionReconcileRef.current = stateKey;
      return;
    }
    
    console.log('[철거공사 Reconcile] 필요:', Array.from(requiredDemolitionWorkNames));
    console.log('[철거공사 Reconcile] 현재 (자동):', Array.from(autoGeneratedDemolitionWorkNames));
    console.log('[철거공사 Reconcile] 누락:', missingDemolitionWorkNames);
    console.log('[철거공사 Reconcile] 불필요:', orphanedDemolitionWorkNames);
    
    // ref 먼저 업데이트 (setLaborCostRows 호출 전에 설정하여 무한 루프 방지)
    demolitionReconcileRef.current = stateKey;
    
    setLaborCostRows(prev => {
      let updatedRows = [...prev];
      
      // 5. 불필요한 자동생성 철거공사 행 제거 (수동 추가된 행은 유지)
      if (orphanedDemolitionWorkNames.length > 0) {
        updatedRows = updatedRows.filter(row => {
          // 수동 추가 행은 유지
          if (!row.isLinkedFromRecovery) return true;
          // 철거공사가 아니면 유지
          if (row.category !== '철거공사') return true;
          // 이 공사명이 orphaned 목록에 있으면 제거
          if (orphanedDemolitionWorkNames.includes(row.workName)) {
      ...
      missingDemolitionWorkNames.forEach(workName => {
        // 해당 공사명의 첫 번째 복구면적 행 찾기 (면적 계산용)
        const sourceAreaRow = rows.find(r => r.workName === workName && r.workType !== '철거공사');
        if (!sourceAreaRow) return;
        
        const damageAreaValue = Number(sourceAreaRow.repairArea) || 0;
        ...
        console.log('[철거공사 Reconcile] 일위대가 조회:', workName, demolitionCatalogItems.length, '개 매칭');
        
        if (demolitionCatalogItems.length > 0) {
          // 일위대가DB에서 매칭된 모든 노임항목으로 철거공사 행 생성
          demolitionCatalogItems.forEach((catItem, idx) => {
            const D = catItem.기준작업량 || 0;
            const E = catItem.노임단가 || 0;
            const C = damageAreaValue;
            
            // 중복 체크: 같은 철거공사+공사명+노임항목이 이미 있으면 건너뛰기
            const exists = updatedRows.some(r => 
              r.category === '철거공사' && 
              r.workName === workName && 
              r.detailItem === catItem.노임항목
            );
            if (exists) {
              console.log('[철거공사 Reconcile] 중복 건너뛰기:', workName, catItem.노임항목);
              return;
            }
            ...
            // ID 형식: demolition-<sourceRowId> (기존 동기화 로직과 호환)
            newDemolitionRows.push({
              id: `labor-demolition-${Date.now()}-${Math.random()}-${idx}`,
              sourceAreaRowId: `demolition-${sourceAreaRow.id}`,
              isLinkedFromRecovery: true,
              sourceWorkType: sourceAreaRow.workType,
              place: sourceAreaRow.category || '',
              position: sourceAreaRow.location || '',
              category: '철거공사',
              workName: workName,
              detailWork: '일위대가',
            ...
            console.log('[철거공사 Reconcile] 생성:', '철거공사', workName, catItem.노임항목);
          });
        } else {
          // 일위대가DB에 없으면 기본 철거공사 행 생성
          const exists = updatedRows.some(r => 
          ...
          if (!exists) {
            ...
            console.log('[철거공사 Reconcile] 생성 (DB 없음):', '철거공사', workName);
          }
        }
      });
  ...
  }, [rows, laborCostRows, isLossPreventionCase, ilwidaegaCatalog]); // laborCostRows 포함 (수동 삭제 감지)
