...
export default function FieldEstimate() {
  ...
  const syncMaterialFromRecoveryArea = () => {
    ...
    // 기존 행 분류: 독립 행(수동 추가), 사용자 수정 행(isOverridden=true), 자동 생성 행
    const independentRows = materialRows.filter(row => !row.isAutoGenerated && !row.isLinkedFromRecovery);
    const overriddenRows = materialRows.filter(row => row.isAutoGenerated && row.isOverridden);
    ...
    // 공사명별 자재 항목 생성 (자재비DB 매칭)
    // Map으로 autoKey 기준 merge: 동일한 autoKey는 1행만 유지
    const autoRowsMap = new Map<string, MaterialRow>();
    ...
    workMap.forEach((data) => {
      ...
      // 수량 계산 (단위별 산식 적용)
      // EA 단위: 합판(1.65m²), 석고(1.62m²), 몰딩(2.44m), 걸레받이(2.44m) → 올림
      // m² 단위: 도배, 마루, 장판 → 그대로 사용
      ...
      if (ratio) {
        // EA 단위 항목: 전체 합산 후 마지막에 한 번만 ceil
      ...
      // 자재항목별로 행 생성 (매칭되는 모든 자재)
      if (matchingMaterials.length > 0) {
        matchingMaterials.forEach(material => {
          const autoKey = `${data.공종}|${data.공사명}|${material.자재항목}`;
          const unitPrice = typeof material.금액 === 'number' ? material.금액 : 0;
          
          // 이미 사용자가 수정한 행(isOverridden)은 기존 값 유지
          const existingOverridden = overriddenRows.find(r => r.autoKey === autoKey);
          if (existingOverridden) {
            // 사용자 수정 행은 autoQuantity만 업데이트하고 나머지는 유지
            autoRowsMap.set(autoKey, {
              ...existingOverridden,
        ...
        // 매칭되는 자재가 없으면 빈 행 생성
        const autoKey = `${data.공종}|${data.공사명}|`;
    ...
    // 자동 생성 행 배열로 변환
    const newAutoRows = Array.from(autoRowsMap.values());
    
    // 최종 병합: 자동 생성 행 + 독립 행 (기존 자동 행은 모두 새로 생성된 것으로 대체)
    const allRows = [...newAutoRows, ...independentRows];
    ...
    if (allRows.length > 0) {
      ...
      toast({
        title: "자재비 동기화 완료",
        description: `복구면적 산출표에서 ${newAutoRows.length}개 항목이 자동 생성되었습니다.`,
      });
    }
  };
